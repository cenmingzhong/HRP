    ...
    $("#tb").toolbar({
        items: [{
            ...
            handler: function () {
                ...
                var personCode = selRow.personTypeCode;
                openDialog({
                    dialogHeight: 450,
                    dialogWidth: 600,
                    url: "toInfo.do?personTypeCode=" + encodeURI(personTypeCode),
            ...
            handler: function () {
                ...
                confirmMsg("确定要删除吗？",function(){
                    ...
                    for(var i = 0 ; i < selRows.length ; i++){
                        personTypeCodes.push(selRows[i].personTypeCode)
                    }

                    ajaxSubmit({
                        url: contextPath + "/db/personType/delete.do",
                        data: {
                            personTypeCodes: JSON.stringify(personTypeCodes)
                        },
            ...
            menuList: [{
                ...
                iconCls: "icon-down",
                handler: function () {
                    downImportTemplate("personType","人员导入模板");
                }
            }]
        }, '-', {
            id: "search",
...
//加载人员数据
function loadPersonTypeList(data) {
   var data={};
    ajaxSubmit({
        url: contextPath + "/db/personType/getList.do",
        data: {
            filter: JSON.stringify(data)
        },
        success: function (result) {
            if (result.isOk == "Y") {
                $('#dataGrid').datagrid({ loadFilter: pagerFilter }).datagrid('loadData', result.data.personTypeList);
            }
        }
    });
}
...
function uploadData() {
    uploadFile({
        ...
        success: function (result) {
            if (result.isOk == "Y") {
                ajaxSubmit({
                    url: contextPath + "/db/personType/importPerson.do",
                    data: {
