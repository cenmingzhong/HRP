    ...
    window.setTimeout(function(){
        ajaxSubmit({
            ...
            success: function (result) {
                if (result.isOk == "Y") {
                    window.setTimeout(function(){
                        initGridData(result.data.vouchTableWorkFlow);
                    }, 100);
                }
            }
        });
...
function initGridData(dataRows){
    var workFlowData = {};
    var workFlowItemList =[]

    var dialogParams= getDialogParams();
    if(dialogParams != null){
        workFlowData = dialogParams.workFlowData;
        workFlowItemList = dialogParams.workFlowItemList;
    }
    $("#tableForm").fill(workFlowData)

    if(workFlowItemList == null || workFlowItemList.length==0){
        $("#itemGrid").datagrid("loadData", dataRows);
    }else{
        var newList = [];
        for(var j = 0 ; j < workFlowItemList.length; j++){
            for(var i = dataRows.length-1 ; i >=0 ; i--){
                if(dataRows[i].fieldCode == workFlowItemList[j].itemName){
        ...
        for(var i = 0 ; i < workFlowItemList.length; i++){
            var idx = $("#itemGrid").datagrid("getRowIndex", workFlowItemList[i].itemName);

            if (idx != -1) {
                $("#itemGrid").datagrid("checkRow", idx);
                dataRows[idx].itemData = workFlowItemList[i];
...
function saveClick(){
    ...
    var workFlowData = $("#tableForm").form2json();
    var $chks = $("#tableForm").find(":checkbox:not(:checked)");    //取得所有未选中的checkbox
    if ($chks.length >= 0) {
        $chks.each(function () {
            var chkName = $(this).attr("name");

            if (workFlowData[chkName] == null) {
                workFlowData[chkName] = "N"
            }
        });
    }
    closeDialog({
        workFlowData:workFlowData,
        workFlowItemList: itemList
    });
}
var selRow=-1;
