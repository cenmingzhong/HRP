...
$.extend($.jgrid,{
    ...
    msiever : function () {
        var rv =0,sAgent = window.navigator.userAgent,Idx = sAgent.indexOf("MSIE");
...
$.fn.jqGrid = function( pin ) {
    ...
    return this.each( function() {
            ...
            dragEnd: function( events ) {
                ...
                if(this.resizing) {
                    ...
                    if(events) {
                        ...
                        if($.isFunction(p.resizeStop)) { p.resizeStop.call(ts,p.colModel[idx],nw,idx); }
                    }
                }
                this.curGbox = null;
        ...
        addJSONData = function(data, rcnt, more, adjust) {
            ...
            var rn = parseInt(ts.p.rowNum,10),selected=false, selr;
            if (adjust) { rn *= adjust+1; }
            ...
            while (i<len) {
                ...
                //if(ts.p.scroll==1){
                    ts.p._index[$.jgrid.stripPref(ts.p.idPrefix, idr)] = (parseInt(ts.p.page,10)-1)*parseInt(ts.p.rowNum,10)+i;
                //}
                if(ts.p.gridview === false ) {
                    tablebody.append(rowData.join(''));
            ...
            if(!ts.grid.hDiv.loading) {
                ...
                case "clientside":
                    ...
                    window.setTimeout(function(){
                        ...
                        endReq();
                        ts.p._ald = false;
                        if(ts.p.scroll && getHeadCheckBox()) {
        ...
        getHeadCheckBox = function ( checked ) {
            return $('#cb_'+$.jgrid.jqID(ts.p.id),ts.grid.hDiv)[ts.p.useProp ? 'prop': 'attr']("checked");
//            var fid = ts.p.frozenColumns ? ts.p.id+"_frozen" : "";
//            if(fid) {
//                $('#cb_'+$.jgrid.jqID(ts.p.id),ts.grid.fhDiv)[ts.p.useProp ? 'prop': 'attr']("checked", checked);
//            }
        ...
        setPager = function (pgid, tp){
            ...
            var sep = "<td style='padding:0px 5px'><span class='ui-separator'></span></td>",
            pginp = "",
            ...
            $("#"+$.jgrid.jqID(pgid) )
            .append("<div id='"+pgcnt+"' class='ui-pager-control' role='group'><table " + getstyle(stylemodule, 'pagerTable', false, 'ui-pg-table ui-common-table ui-pager-table') + "><tbody><tr><td id='"+cent+"' align='center' style='white-space:pre;'></td><td id='"+rgt+"' align='right'></td></tr></tbody></table></div>")
            ...
            if(ts.p.pgbuttons===true) {
                ...
                var pageIcon = "ui-pager"
                pgl += "<td id='"+po[0]+"' class='"+btc+"' title='"+ pot[0] +"'" + "><span " + getstyle(stylemodule, 'icon_first', false, pageIcon)+"></span></td>";
                pgl += "<td id='"+po[1]+"' class='"+btc+"'  title='"+ pot[1] +"'" +"><span " + getstyle(stylemodule, 'icon_prev', false, pageIcon)+"></span></td>";
                pgl += pginp !== "" ? sep+pginp+sep:"";
                pgl += "<td id='"+po[2]+"' class='"+btc+"' title='"+ pot[2] +"'" +"><span " + getstyle(stylemodule, 'icon_next',false, pageIcon)+"></span></td>";
                pgl += "<td id='"+po[3]+"' class='"+btc+"' title='"+ pot[3] +"'" +"><span " + getstyle(stylemodule, 'icon_end',false, pageIcon)+"></span></td>";
            } else if (pginp !== "") { 
                pgl += pginp; 
            }
            if(dir==="ltr") { 
        ...
        for(i=0;i<this.p.colNames.length;i++){
            ...
            thead += "<div class='ui-th-div "+(tmpcm.isRequired===true?"needLabel":"")+"' id='jqgh_"+ts.p.id+"_"+tmpcm.name+"' "+tdc+">"+ts.p.colNames[i];
            if(!tmpcm.width)  { 
                tmpcm.width = 150; 
            } else { 
        ...
        $("th",thr).each(function ( j ) {
            ...
            if(ts.p.footerrow) {
                if(nm==='rn'){
                    tfoot += "<td role='gridcell'  style='width:34px;text-align:center' class='jqgrid-rownum ui-state-default'>合计</td>"; 
                }else{
        ...
        if(ts.p.pagination){
            ...
            setPager(ts.p.pager,'');
            */
            var pagerId = 'pager_'+ts.id
            ts.p.pager = "#"+pagerId;
            
            $("<div id='"+pagerId+"'></div>").css({width: grid.width+"px"}).appendTo(eg).addClass('ui-state-default ui-jqgrid-pager ui-corner-bottom');
            if(hg) {$(ts.p.pager).hide();}
            setPager(ts.p.pager,'');
        }
        ...
        $(ts).before(grid.hDiv).on({
            'click': function(e) {
                ...
                $(".ui-state-highlight", ts).removeClass("ui-state-highlight");
                $(ptr).addClass("ui-state-highlight");
                if ($.isFunction(ts.p.onClickRow)) { 
                    ts.p.onClickRow.call(ts,$(ptr).attr("id"),ptr[0].rowIndex,ci); 
                }
                ...
                if (td.length > 0) {
                    ...
                    if($.isFunction(ts.p.onCellSelect)) {
                        if(ts.p.checkOnSelect ==false &&ci!=1){
                            $(".ui-state-highlight", ts).removeClass("ui-state-highlight");
                            $(ptr).addClass("ui-state-highlight");
                        }
                        cSel = ts.p.onCellSelect.call(ts,ri,ci,tdHtml,e);
                    }
                }
                if (!cSel) {
                    return;
            ...
            'keyDown':function(e){
                alert(1)
                if ($.isFunction(ts.p.onKeyDown)) { 
                    ptr = $(e.target,ts.rows).closest("tr.jqgrow");
                    ri = ptr[0].id;
                    ts.p.onKeyDown.call(ts,e,ri,ptr[0].rowIndex); 
                }
            },
        ...
        if( ts.p.toolbar[0] ) {
            ...
            if(ts.p.toolbar[1]==="both") {
                ...
                $(grid.uDiv).addClass( tbstyle + " ui-userdata-top").attr("id","t_"+this.id);
            }
            
            if (ts.p.toolbar[2]) {
                $(grid.uDiv).toolbar({items:ts.p.toolbar[2],backgournd:"#F4F4F4"});
...
//                        } else {
//                            var tool = $("<a href=\"javascript:void(0)\" class='fm-button ui-state-default ui-corner-all ui-reset fm-button-icon-left'>" + item.text + "</a>").appendTo($(grid.uDiv));
//                            tool[0].onclick = eval(item.handler || function () { }); tool.css("float" , "left");
//                            //tool.linkbutton($.extend({}, p, { plain: true }));tool.append("<div class='" + p.iconCls + "'>" + p.text + "</div>"); tool.append(p.text);  --������ҪEasyUI��ܺ���ʽ��֧��
//                        }
//                    }
...
//                }
        ...
        ts.getHeadCheckBox = getHeadCheckBox;
        ...
        if(ts.p.fit){
            var setFitHeight = function(){
                var totalHeight = $("#gbox_"+self.id).parent().height();
                var pageHeight = $("#gbox_"+self.id+" .ui-jqgrid-pager").outerHeight()
                var footerHeight = $("#gbox_"+self.id+" .ui-jqgrid-sdiv").outerHeight()
                var headHeight = $("#gbox_"+self.id+" .ui-jqgrid-hdiv").outerHeight();
                $("#"+self.id).setGridWidth($("#gbox_"+self.id).parent().width());
                $("#gbox_"+self.id+" .ui-jqgrid-bdiv").height(totalHeight-pageHeight-headHeight-footerHeight-2-(ts.p.toolbar[0]===true?32:0))
            }
            $("#gbox_"+self.id).parent().resize(function(){   
                $("#"+self.id).setGridWidth(0);
                $("#"+self.id).setGridHeight(0);
                window.setTimeout(function(){
                    setFitHeight();
                },1)
            });
...
$.jgrid.extend({
    loadData : function(data,clearRow) {
        return this.each(function(){
            var gridSetting = $(this).jqGrid("getGridParam","gridSetting");
            
            if(data != null){
                for(var i = 0 ; i < data.length; i++){
                    data[i].jqRowId_ = undefined;
                }
            }
            this.setHeadCheckBox( false);
            if(gridSetting.groupOption != null){
                var gridId = this.id;
                
                if(gridSetting.groupOption.originalColumns){
                    gridSetting.columns = gridSetting.groupOption.originalColumns;
                }
                $.jgrid.gridUnload(gridId);
                gridSetting.data = data;
                gridSetting.groupOption = null;
                newJgGrid(gridId, gridSetting);
            }else{
                $(this).jqGrid('clearGridData');
                clearRow = clearRow== null?true:clearRow;
                
                if(clearRow && this.grid.p != null && this.grid.p.delRows!=null){
                    this.grid.p.delRows = null;
                }
                $(this).jqGrid('setGridParam',{clickRowId:null});
                this.grid.p.selarrrow = [];
                this.grid.p.data = data==null?[]:data;
                this.grid.p.sortname='';
                
                var previousSelectedTh =this.grid.headers[this.grid.p.lastsort].el;
                var tmpicon = $(previousSelectedTh).find("span.ui-grid-ico-sort");
                tmpicon.addClass("ui-state-disabled");
                this.grid.populate();
            }
        });
    },
    getData : function(isSaveData) {
        var data = null;
        this.each(function(){
            if ((isSaveData==null || isSaveData!=false) && this.grid.p.savedRow.length>0) {
                $(this).jqGrid("saveCell",this.grid.p.savedRow[0].id,this.grid.p.savedRow[0].ic);
            } 
            data = this.grid.p.data;
        });
        return data;
    },
    getAllData : function() {
        var data = null;
        this.each(function(){
            data = $(this).jqGrid("getData");
            if(this.grid.p.delRows!=null){
                data = data.concat(this.grid.p.delRows);
            }
        });
        return data;
    },
    getSelectedId : function() {
        var ids = null;
        this.each(function(){
            if (this.grid.p.savedRow.length>0) {
                $(this).jqGrid("saveCell",this.grid.p.savedRow[0].id,this.grid.p.savedRow[0].ic);
            } 
            ids=$(this).jqGrid('getGridParam','selrow');
        });
        return ids;
    },
    getSelectedIds : function() {
        ...
        this.each(function(){
            ...
            ids=$(this).jqGrid('getGridParam','selarrrow');
        });
        return ids;
    },
    //获取选中行，如果未选中，多选情况下返回空数组，单选情况下返回null
    getSelection : function() {
        var selection = null;
        this.each(function(){
            if(this.grid.p.multiselect){
                if(this.getHeadCheckBox()){
                    selection = $(this).jqGrid("getData");
                }else{
                    selection = [];
                    var ids= $(this).jqGrid("getSelectedIds");
                    for(var i = 0 ; i < ids.length ; i++){
                        selection.push($(this).jqGrid("getRowData", ids[i],true))
                    }
                }
            }else{
                var id= $(this).jqGrid("getSelectedId");
                if(id != null){
                    selection = $(this).jqGrid("getRowData", id,true);
                }
            }
        });
        return selection;
    },
    saveDelRow : function(rowData) {
        this.each(function(){
            if(this.grid.p.delRows==null){
                this.grid.p.delRows = [];
            }
            rowData.isDelete=true;
            this.grid.p.delRows.push(rowData)
        });
    },
    getColumnList: function(){
        ...
        this.each(function(){
            var gridSetting = $(this).jqGrid("getGridParam","gridSetting");
            colList = gridSetting.columns;
            if(gridSetting.groupOption != null && gridSetting.groupOption.originalColumns){
                colList = gridSetting.groupOption.originalColumns;
            }
    ...
    exportExcel : function(options) {
        this.each(function(){
            ...
            if(dataList == null){
                dataList = $(this).jqGrid("getData");
            }
            var gridSetting = $(this).jqGrid("getGridParam","gridSetting");

            var cols = gridSetting.columns;
            var colList = [];
        
            for (var i = 0; i < cols.length; i++) {
                if(cols[i].code =="operator" || cols[i].isExport==false){
                    continue;
                }
                var text = cols[i].text;
                text = text.replace("<b style='font-size:14px'>",'');
                text = text.replace("</b>",'');
                colList.push({
                    colId: cols[i].code,
                    colField: cols[i].col,
                    ...
                    isHidden: cols[i].isExport==true?false:cols[i].hidden,
                    colFormatter:cols[i].isExportFmt==false?null:cols[i].formatter,
                    isSum:cols[i].isSum
                })
            }
            var groupHeaderList = [];
            var tempList = this.p.groupHeader;
            if(tempList != null){
                for(var i = 0 ; i < tempList.length; i++){
                    groupHeaderList.push(tempList[i].groupHeaders);
                }
            }
            outDtExcel({
                ...
                titleData:JSON.stringify(options.titleData),
                groupHeaderList:JSON.stringify(groupHeaderList),
                footData:JSON.stringify(options.footData),
                mergeList:JSON.stringify(options.mergeList)
            });
        });
    },
    ...
    resetSelection : function( rowid ){
        return this.each(function(){
            ...
            if(rowid !== undefined ) {
                ...
                $(".ui-state-highlight", t).removeClass("ui-state-highlight");
                t.setHeadCheckBox( false );
                t.p.selarrrow = [];
                t.p.selrow = null;
            }
    ...
    delRowData : function(rowid) {
        ...
        this.each(function() {
            ...
            if($t.p.datatype === 'local') {
                ...
                if($.isFunction($t.p.loadComplete)){
                    $t.p.loadComplete.call($t,null);
                }
            }
        });
        return success;
                ...
                try {
                    $(this.p.colModel).each(function(i){
                        ...
                        if(nm!="rn" && nm!="cb"){
                            ...
                            //if( dval !== undefined) {
                                dval = this.formatter && typeof this.formatter === 'string' && this.formatter === 'date' ? $.unformat.date.call(t,dval,this) : dval;
                                vl = t.formatter( rowid, dval, i, data, 'edit');
                                title = this.title ? {"title":$.jgrid.stripHtml(vl)} : {"title":""};
                                
                            //}
                            if(t.p.treeGrid===true && nm === t.p.ExpandColumn) {
                                $("td[role='gridcell']:eq("+i+") > span:first",ind).html(vl).attr(title);
                            } else {
                                var td = $("td[role='gridcell']:eq("+i+")",ind);
                                if(!td.hasClass("edit-cell")){
                                    td.html(vl).attr(title);
                                }else{
                                    td.find("input[type='text']").val(vl)
                                }
                            }
                        }
                    });
    ...
    addRowData : function(rdata,rowid,pos,src) {
        ...
        if(rdata) {
            ...
            this.each(function() {
                ...
                if(!aradd) {
                    if(rowid!=null && rowid !== undefined) { rowid = String(rowid);}
                    else {
                        rowid = $.jgrid.randId();
                    ...
                    if(t.p.datatype === 'local') {
                        ...
                        switch (pos) {
                            case 'last':
                                ...
                                break;
                            case 'first':
                                t.p.data.splice(0, 0, data);
                                break;
                            case 'after':
                                sind = $(t).jqGrid('getGridRowById', src);
                                if (sind) {
                                    t.p.data.splice(sind.rowIndex, 0, data);
                                }    
                                t.refreshIndex()
                                break;
                            case 'before':
                                sind = $(t).jqGrid('getGridRowById', src);
                                if (sind) {
                                    t.p.data.splice(sind.rowIndex-2, 0, data);
                                } 
                                t.refreshIndex()
                                break;
                        }
    ...
    hideCol : function (colname) {
        return this.each(function(){
            ...
            var gridSetting = $(this).jqGrid("getGridParam","gridSetting");
            if(gridSetting != null && gridSetting.columns != null){
                for(var i = 0 ; i < gridSetting.columns.length; i++){
                    if(gridSetting.columns[i].code == colname){
                        gridSetting.columns[i].hidden = true;
                        break;
                    }
                }
            }
    ...
    showCol : function(colname) {
        return this.each(function(){
            ...
            if(gridSetting != null && gridSetting.columns != null){
                for(var i = 0 ; i < gridSetting.columns.length; i++){
                    if(gridSetting.columns[i].code == colname){
                        gridSetting.columns[i].hidden = false;
                        break;
                    }
                }
            }
    ...
    setGridWidth : function(nwidth, shrink) {
        return this.each(function(){
            ...
            if($t.p.toolbar[0] === true){
                $($t.grid.uDiv).css("width",(nwidth - bstw-6)+"px");
                ...
                if (ind){
                    ...
                    if(nData !== "" || forceupd === true ) {
                        ...
                        if($t.p.datatype === "local") {
                            ...
                            nData = cm.formatter && typeof cm.formatter === 'string' && cm.formatter === 'number' ? v : nData;
                            index = $t.p._index[$.jgrid.stripPref($t.p.idPrefix, rowid)];
                            if(index !== undefined) {
                                
                                if(cm.name.indexOf(".")>-1){
                                    var prm = cm.name.split('.');
                                    var dataObj = $t.p.data[index];
                                    for(var i = 0 ; i < prm.length-1; i++){
                                        dataObj = $t.p.data[index][prm[i]];
                                        if(dataObj == null){
                                            dataObj = new Object();
                                        }
                                    }
                                    if(nData!=""){
                                        dataObj[prm[prm.length-1]]= nData;
                                    }else{
                                        dataObj = $t.p.data[index];
                                        dataObj[prm[prm.length-2]]= null;
                                    }
                                    
                                }else{
                                    $t.p.data[index][cm.name] = nData;
                                }
                                $t.p.data[index].isUpdate= true;
                            }
                        }
                    }
...
//                    }
//                }
                var dataList = $t.p.data;
                dataList=$.grep(dataList,function(obj,i){
                    return obj.jqGroupData==null || obj.jqGroupData == false;
                });
                var ln = dataList.length, i =0, dlen=0;
                if (ln && ln>0){
                    while(i<ln){
                        try {
                            val = $.jgrid.getAccessor(dataList[i],col) 
                        } catch (e) {
                            val = 0;
                        }
                        if(mathopr) {
...
$.jgrid.extend({
    editCell : function (iRow,iCol, ed){
        return this.each(function (){
            ...
            var rowData = $($t).jqGrid("getRowData",$t.rows[iRow].id,true);
            if (cm.editable===true && ed===true && !cc.hasClass("not-editable-cell") && (!$.isFunction($t.p.isCellEditable) || $t.p.isCellEditable.call($t,nm,rowData,iRow,iCol))) {
                if(parseInt($t.p.iCol,10)>=0  && parseInt($t.p.iRow,10)>=0) {
                    //$("td:eq("+$t.p.iCol+")",$t.rows[$t.p.iRow]).removeClass("edit-cell " + highlight);
                ...
                window.setTimeout(function () { $(elc).focus().select();},1);
                    ...
                    if(e.keyCode=="37"){
                            ...
                            if($(this).attr("isrefer") != "true"){
                                ...
                                window.setTimeout(function(){
                                    if(iRow==1){
                                        ...
                                        $($t).jqGrid("prevCell",iRow-1,iCol+1);
                                    }
                        ...
                        if(!$t.grid.hDiv.loading ) {
                            if(iRow<$t.rows.length-1){
                               ...
                               if($(this).attr("isrefer") != "true"){
                                    $(elc).blur();
                                    $($t).jqGrid('addRowData',{ isNew: true})
                                    window.setTimeout(function(){
                                        var colIdx = 0;
                                        for(var i = 0 ; i < $t.p.colModel.length; i++){
                                            if($t.p.colModel[i].editable){
                                                colIdx = i;
                                                break;
                                            }
                                        }
                                        $($t).jqGrid("editCell",iRow+1,colIdx,true);
                                    },2)
                                }
                            }
                            return false;
                ...
                $("input, select, textarea",cc).on("blur",function(e) {
                    //$($t).jqGrid("saveCell",iRow,iCol);
                });
                $($t).triggerHandler("jqGridAfterEditCell", [$t.rows[iRow].id, nm, tmp, iRow, iCol]);
                if ($.isFunction($t.p.afterEditCell)) {
                    $t.p.afterEditCell.call($t, tmp, $t.rows[iRow],nm,iRow,iCol);
                }
            } else {
                if (parseInt($t.p.iCol,10)>=0  && parseInt($t.p.iRow,10)>=0) {
                    //$("td:eq("+$t.p.iCol+")",$t.rows[$t.p.iRow]).removeClass("edit-cell " + highlight);
                ...
                if (v2 !== $t.p.savedRow[fr].v){
                    ...
                    if(cv[0] === true) {
                        ...
                        if ($t.p.cellsubmit === 'clientArray') {
                            $(cc).empty();
                            $($t).jqGrid("setCell",$t.rows[iRow].id,iCol, v2, false, false, true);
                            $(cc).addClass("dirty-cell").removeClass("edit-cell");
                            $($t.rows[iRow]).addClass("edited");
                            $($t).triggerHandler("jqGridAfterSaveCell", [$t.rows[iRow].id, nm, v, iRow, iCol]);
                            if ($.isFunction($t.p.afterSaveCell)) {
                                $t.p.afterSaveCell.call($t, $t.rows[iRow].id,nm, v, iRow,iCol);
    ...
    nextCell : function (iRow,iCol) {
        return this.each(function (){
            ...
            for (i=iCol+1; i<$t.p.colModel.length; i++) {
                if ( $t.p.colModel[i].editable ===true && (!$.isFunction($t.p.isCellEditable) || $t.p.isCellEditable.call($t,$t.p.colModel[i].name,rowData, iRow,i))) {
                    nCol = i; break;
                }
            }
            if(nCol !== false) {
                $($t).jqGrid("editCell",iRow,nCol,true);
            }else if(iRow<$t.p.reccount){
                iRow++;
                rowData = $($t).jqGrid("getRowData",$t.rows[iRow].id,true);
                for (i=1; i<$t.p.colModel.length; i++) {
                    if ( $t.p.colModel[i].editable ===true && (!$.isFunction($t.p.isCellEditable) || $t.p.isCellEditable.call($t,$t.p.colModel[i].name,rowData, iRow,i))) {
                        nCol = i; break;
                    }
                }
    ...
    prevCell : function (iRow,iCol) {
        return this.each(function (){
            ...
            if(nCol !== false) {
                ...
                iRow--;
                rowData = $($t).jqGrid("getRowData",$t.rows[iRow].id,true);
                for (i=$t.p.colModel.length-1; i>=1; i--) {
                    if ( $t.p.colModel[i].editable ===true && (!$.isFunction($t.p.isCellEditable) || $t.p.isCellEditable.call($t, $t.p.colModel[i].name,rowData, iRow,i))) {
                        nCol = i; break;
                    }
                }
...
//Helper functions
    ...
    bindEv: function  (el, opt) {
        ...
        if($.isFunction(opt.dataInit)) {
            var rowData = $($t).jqGrid("getRowData",opt.rowId,true);
            opt.dataInit.call($t,el,opt,rowData);
        }
        if(opt.dataEvents) {
    ...
    createEl : function(eltype,options,vl,autowidth, ajaxso) {
        ...
        switch (eltype)
        {
            ...
            default :
                ...
                if(eltype !== "button"){
                    if(autowidth) {
                        if(!options.size) { $(elem).css({width:"99%"}); }
...
$.jgrid.extend({
                                ...
                                if(postdata[oper] === opers.addoper ) {
                                    ...
                                    if(rp_ge[$t.p.id].reloadAfterSubmit) {
                                        ...
                                        if($t.p.treeGrid === true){
                                            ...
                                            $($t).jqGrid("addRowData",postdata,ret[2],p.addedrow);
                                        }
                                    }
    ...
    FormToGrid : function(rowid, formid, mode, position){
        return this.each(function() {
            ...
            if(mode==='add') {$($t).jqGrid("addRowData",griddata,rowid, position);}
...
$.jgrid.extend({
    ...
    setGroupHeaders : function ( o ) {
        ...
        return this.each(function(){
            ...
            if(ts.p.fit){
                $("#"+ts.id).setGridWidth(0);
                $("#"+ts.id).setGridHeight(0);
            }
            ...
            if(ts.p.fit){
                ...
                var setFitHeight = function(){
                    ...
                    var headHeight = $("#gbox_"+self.id+" .ui-jqgrid-hdiv").outerHeight();
                    $("#"+self.id).setGridWidth($("#gbox_"+self.id).parent().width());
                    $("#gbox_"+self.id+" .ui-jqgrid-bdiv").height(totalHeight-pageHeight-headHeight-footerHeight-1-(ts.p.toolbar[0]===true?32:0))
                }
                $("#gbox_"+self.id).parent().resize(function(){   
                    $("#"+self.id).setGridWidth(0);
                    $("#"+self.id).setGridHeight(0);
...
//Editing
    ...
    addRow : function ( p ) {
        ...
        return this.each(function(){
            ...
            $($t).jqGrid('addRowData', p.initdata,p.rowID, p.position);
                ...
                return $.extend({
                    ...
                    drop: function(ev, ui) {
                        ...
                        if(ui.helper.dropped) {
                            ...
                            $("#"+$.jgrid.jqID(this.id)).jqGrid('addRowData',getdata,grid,opts.droppos);
                        }
    ...
    addChildNode : function( nodeid, parentid, data, expandData ) {
        ...
        if(data) {
            ...
            if( parentid === null || $($t).jqGrid("isNodeLoaded",parentdata) || leaf ) {
                    $($t).jqGrid('addRowData',  data,nodeid, method, rowind);
