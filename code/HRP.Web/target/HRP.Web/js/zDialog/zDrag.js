var Drag = {
    ...
    "end": function () {
        ...
        var st = Math.max(document.documentElement.scrollTop, document.body.scrollTop); //考虑滚动的情况

        if (parseInt(Drag.obj.root.style.top) - st < 0)
            Drag.obj.root.style.top = st + "px";
        else if (parseInt(Drag.obj.root.style.top) - st > ch - Drag.obj.root.offsetHeight)
            Drag.obj.root.style.top = (ch - Drag.obj.root.offsetHeight + st) + "px";
            
        if (parseInt(Drag.obj.root.style.left) - sl < 0)
            Drag.obj.root.style.left = sl+"px";
        else if (parseInt(Drag.obj.root.style.left) - sl > cw - Drag.obj.root.offsetWidth)
            Drag.obj.root.style.left = (cw - Drag.obj.root.offsetWidth + sl) + "px";
