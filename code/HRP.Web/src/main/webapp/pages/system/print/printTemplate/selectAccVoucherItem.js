    ...
    window.setTimeout(function(){
        ajaxSubmit({
            ...
            success: function (result) {
                if (result.isOk == "Y") {
                    window.setTimeout(function(){
                        initGridData(result.data.vouchTableAccVoucher);
                    }, 100);
                }
            }
        });
...
function initGridData(dataRows){
    var accVoucherData = {};
    var accVoucherItemList =[]

    var dialogParams= getDialogParams();
    if(dialogParams != null){
    	accVoucherData = dialogParams.accVoucherData;
    	accVoucherItemList = dialogParams.accVoucherItemList;
    }
    $("#tableForm").fill(accVoucherData)

    if(accVoucherItemList == null || accVoucherItemList.length==0){
        $("#itemGrid").datagrid("loadData", dataRows);
    }else{
        var newList = [];
        for(var j = 0 ; j < accVoucherItemList.length; j++){
            for(var i = dataRows.length-1 ; i >=0 ; i--){
                if(dataRows[i].fieldCode == accVoucherItemList[j].itemName){
        ...
        for(var i = 0 ; i < accVoucherItemList.length; i++){
            var idx = $("#itemGrid").datagrid("getRowIndex", accVoucherItemList[i].itemName);

            if (idx != -1) {
                $("#itemGrid").datagrid("checkRow", idx);
                dataRows[idx].itemData = accVoucherItemList[i];
...
function saveClick(){
    ...
    var accVoucherData = $("#tableForm").form2json();
    var $chks = $("#tableForm").find(":checkbox:not(:checked)");    //取得所有未选中的checkbox
    if ($chks.length >= 0) {
        $chks.each(function () {
            var chkName = $(this).attr("name");

            if (accVoucherData[chkName] == null) {
            	accVoucherData[chkName] = "N"
            }
        });
    }
    closeDialog({
    	accVoucherData:accVoucherData,
        accVoucherItemList: itemList
    });
}
var selRow=-1;
